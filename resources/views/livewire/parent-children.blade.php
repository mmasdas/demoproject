<form wire:submit="save">
    <div>
        ...
        <input id="customer_name" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" type="text"
            wire:model="customer_name" />
    </div>

    <div class="mt-4">
        <label for="customer_email" class="block font-medium text-sm text-gray-700">Customer email</label>
        <input id="customer_email" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" type="email"
        ...
        <div class="min-w-full align-middle p-4">
            <table class="min-w-full divide-y divide-gray-200 border">
                <thead>
                    <tr>
                        <th class="px-6 py-2 bg-gray-50 text-left">
                            <span
                                class="text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Product</span>
                        </th>
                        <th class="px-6 py-2 bg-gray-50 text-left">
                            <span
                ...
                <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                    ...
                    <tr>
                        <td class="px-2 py-2">
                            <select wire:model="orderProducts.{{ $index }}.product_id"
                        ...
                        <td class="px-2 py-2">
                            <input wire:model="orderProducts.{{ $index }}.quantity"
                                id="orderProducts[{{ $index }}][price]"
                                class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" type="number" />
                        </td>
                        <td class="px-2 py-2">
                            <button type="button" wire:click='removeProduct({{ $index }})'
                                class="px-3 py-2 bg-red-600 rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500">
                                Delete
                            </button>
                        </td>
                    </tr>
